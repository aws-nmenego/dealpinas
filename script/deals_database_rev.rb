...
#      return nil
#    end
    if !var.nil? and var.is_a? String
      var = var.gsub /'/, '\\\\\''
    else
      var =  nil
    end
    ...
    if @dbh.nil?
      ...
      # NOTE: the following is done with the assumption that deal URL does not change
        ...
        sql = "UPDATE `deals` SET 
              `rss_deal_id`='',
              `title`='#{esc deal.title}',
              `description`='#{esc deal.description}',
              `price`='#{esc deal.price}',
              `quantity`='#{esc deal.items}',
              `deal_thumb`='#{esc deal.deal_thumb}',
              `updated_at`=NOW(),
              `no_of_items_sold`='#{esc deal.sold}', 
              `discount` = '#{esc deal.discount}', 
              `author` = '#{esc deal.author}' , 
              `expiry` = '#{esc deal.expiry}' ,
              status = 'old'

...
class PinoyGreatDealsScraper < Scraper
    
  def initialize 
    super ARGV[0],"pinoygreatdeals"
  end  
  ...
  def get_data  deal_html, deal
    ...
    html = deal_html.to_s
    deal[:expiry] = html[/var\s*untilDate\s*=\s*new\s*Date\s*\(\s*\d*\s*\)/]
    deal[:expiry] = deal[:expiry].gsub /var\s*untilDate\s*=\s*new\s*Date\s*\(/ , ''
    deal[:expiry] = deal[:expiry].gsub /\)/ , ''
    deal[:expiry] = Time.at deal[:expiry].to_i/1000
    ...
    links.each do |a|
      if a.content
        if a.content.to_s[/Get\s*Directions/i]
          begin
            loc = a['href'].to_s()[/\?.*/]
            loc = loc.split '&'
            loc = loc[0]
...
PinoyGreatDealsScraper.new.start
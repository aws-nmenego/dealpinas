...
class BuyanihanScraper < Scraper
  
  def initialize 
    super ARGV[0],"buyanihan"
  end
  ...
  def get_data  deal_html, deal
    ...
    links.each do |a|
      if a.content
        if a.content.to_s[/see\s*google\s*map/i]
          loc = a['href'].to_s()[/\?.*/]
          loc = loc.split '&'
          loc = loc[0]
          loc = loc.gsub /\?q=/ , ''
    ...
    deal[:description] = deal[:description].gsub /(\\.?)*/ , ''
    deal[:description] = deal[:description].chomp
